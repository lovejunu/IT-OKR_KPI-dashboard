...
md_path = r'C:\Users\NKIA1\hipo-2\IT OKR_KPI-dashboard\04_신입사원_자기주도학습교재_엔키아이해하기.md'
docx_path = r'C:\Users\NKIA1\hipo-2\IT OKR_KPI-dashboard\04_신입사원_자기주도학습교재_엔키아이해하기.docx'
...
while i < len(lines):
    ...
    if stripped.startswith('# ') and not stripped.startswith('## '):
        doc.add_heading(strip_links(stripped[2:]), level=1)
        i += 1
        continue

    if stripped.startswith('## '):
        doc.add_heading(strip_links(stripped[3:]), level=2)
        i += 1
        continue

    if stripped.startswith('### '):
        doc.add_heading(strip_links(stripped[4:]), level=3)
        i += 1
        continue

    if stripped.startswith('#### '):
        doc.add_heading(strip_links(stripped[5:]), level=4)
        i += 1
        continue
    ...
    if m_check:
        checked, content = m_check.groups()
